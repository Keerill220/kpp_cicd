...
class _AddWaterPageState extends State<AddWaterPage> {
  ...
  void _handleSubmit() {
    ...
    context.read<IntakeHistoryBloc>().add(CreateIntakeRecord(
          time: timeString,
          amount: amount,
          type: _selectedType,
        ));
  ...
  Widget build(BuildContext context) {
    return BlocListener<IntakeHistoryBloc, IntakeHistoryState>(
      listener: (context, state) {
        if (state is IntakeRecordCreating) {
          setState(() => _isLoading = true);
        } else if (state is IntakeRecordCreateSuccess) {
        ...
        } else if (state is IntakeRecordCreateError) {
          setState(() => _isLoading = false);
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
